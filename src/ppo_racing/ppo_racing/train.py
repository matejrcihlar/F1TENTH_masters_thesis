...
def main():
    ...
    alpha = 1e-4
    n_games = 20000
    figure_file = f'plots/ros2_agent_learning_curve_{N}_{batch_size}_{n_epochs}_{timestamp}.png'
    ...
    input_size = scan_size + pose_size_ego + pose_size_opp + vel_size *2 + waypoint_size *2+ collision_size + prev_action_size
    ...
    if load_model:
        agent.load_models(weights_only=True)
    ...
    reward_node = RewardNode(reward_function='combined')
    ...
    for i in range(n_games):
        ...
        observation = preprocess_observation(observation, WAYPOINTS, prev_action, ego=0)
        done = False
        score = 0
        duration = 0.
        ...
        if (i + 1) % 100 == 0:
            ...
            print(f"[Episode {i+1}] Time elapsed: {elapsed_hours}:{elapsed_minutes:.2f}")
        ...
        while not done:
            ...
            action_sized = np.array([np.clip(action[0], -0.34, 0.34), np.clip(action[1], 0.5, 3.0)])
            smoothed_action = beta * prev_action + (1 - beta) * action_sized
            prev_action = smoothed_action
            #opp_action = np.array([0.0, 0.0])
            action_full =np.array([[smoothed_action[0], smoothed_action[1]], [opp_action[0], opp_action[1]]])
            ...
            reward = reward_node.get_reward(obs_, np.array(action_sized), ego=0)
            ...
            #time.sleep(0.5)
            #print("reward:", reward)
            obs_ = preprocess_observation(obs_, WAYPOINTS, prev_action, ego=0)
        ...
        if (i+1)%200 == 0:
            figure_file_rewards = f'reward_plots/Reward of episode:{i+1}.png'
            reward_node.plot_logs(figure_file_rewards,i+1)
...
def reset_poses(flat_waypoints):
    ...
    rand_dist = np.random.randint(3, 30)
...
def get_opp_action(obs, flat_waypoints, L=3.0, K=0.5):
    ...
    velocity = 1.5 - 1 * np.abs(steering_angle)
